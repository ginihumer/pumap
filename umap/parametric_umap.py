...
class ParametricUMAP(UMAP):
    ...
    def _fit_embed_data(self, X, n_epochs, init, random_state):
        ...
        self.head = tf.constant(tf.expand_dims(head.astype('int64') , 0)) # without dtype conversion, I got "ValueError: Failed to convert a NumPy array to a Tensor (Unsupported numpy type: NPY_INT)." error on windows
        self.tail = tf.constant(tf.expand_dims(tail.astype('int64'), 0))
...
def construct_edge_dataset(
    ...
    if parametric_embedding:
#         print(edges_to_exp.dtype, edges_from_exp.dtype) # produces error, if not cast to int64 on windows
        edge_dataset = tf.data.Dataset.from_tensor_slices(
            (edges_to_exp.astype('int64'), edges_from_exp.astype('int64'))
        )
